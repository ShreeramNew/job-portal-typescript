...
export default function PostOrEditJob({
  ...
  const fetchJobDetails = async () => {
    try {
      ...
      let API = "/api/getJobDetails?jobId=" + jobId;
  ...
  const onFinish: FormProps<FieldType>["onFinish"] = async (values) => {
    ...
    try {
      ...
      if (isForEdit) {
        ...
        response = await api.post(API, payload, { withCredentials: true });
      }
...
const FormComponent = ({
  ...
  return (
    <>
      <h3 className="left-0 text-gray-700 font-bold text-[1.2rem] m-6">
        Job Details
      </h3>
